...
router.post('/vendor-rep-reg', function(req, res, next){
    ...
    var repTrans = new VisitHistory ({
        ...
        ,   staffPhoneNo: req.body.hStaff_phone
    });
...
//////  Calling ejs: Menu item Vendor Visit
//////  Provides List of vender representative.
router.get('/vendor-rep-visit', function(req, res, next){
    VendorRep
    .find({},['_id','name','vendorId']).sort('name')
    .exec (function(err, vendorReps){
        ...
        async.each(vendorReps,function(vendorRep,callback){
            Vendor.findById(vendorRep.vendorId, function(err, vendor){
                repDetailsItem = {'repName': vendorRep.name,'repCompId':vendor._id,'repComp':vendor.company,'repAdd':vendor.address, 'repCompPh':vendor.phoneNo, 'repId': vendorRep._id, 'repPhone':vendorRep.phoneNo};
...
////// AJAX response
////// Calling ejs: vendorVisit.ejs
////// On selecting Vendor Representative, send history of visits
...
////// AJAX Response
////// Calling ejs: vendorVisit.ejs
////// On loading of the page to populate Staff Information
...
router.post('/vendor-rep-visit', function(req, res, next){
    var visitData = {
            vendorId    :   req.body.hRep_vendor_id
        ,   staffId     :   req.body.staffList
        ,   vendorName  :   req.body.hRep_vendor_name
        ,   repName     :   req.body.hRep_name
        ,   repPhoneNo  :   req.body.hRep_ph
        ,   staffName   :   req.body.hStaff_name
        ,   staffPhoneNo:   req.body.hStaff_phone
        ,   repId       :   req.body.hRep_id
    };
    console.log('Visit Data to POST: ', visitData);
    VisitHistory.create(visitData, function(err, histInstance){
        if(!err){
            console.log('History created successfully');
            res.redirect('/profile');
        } else {
